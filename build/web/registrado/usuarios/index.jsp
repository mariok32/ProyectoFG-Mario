...
<%!
	...
	*/
    ...
    //Paths, sperated by semicolon
        ...
        private static final String RESTRICT_PATH = "G:\\CloudSpace"; //Win32: Case important!!
	...
	//Button names
	private static final String SAVE_AS_ZIP = "Descargar elementos seleccionados como zip";
	private static final String RENAME_FILE = "Renombrar archivo";
	private static final String DELETE_FILES = "Borrar elementos seleccionados";
	private static final String CREATE_DIR = "Crear directorio";
	private static final String CREATE_FILE = "Crear archivo";
	private static final String MOVE_FILES = "Move archivo";
	private static final String COPY_FILES = "Copiar archivo";
	private static final String LAUNCH_COMMAND = "Ejecutar copias de seguridad";
	private static final String UPLOAD_FILES = "Subir ficheros";
...
<%
		...
		if (request.getParameter("Javascript") != null) {
			...
			response.setHeader("Expires", sdf.format(new java.util.Date(now.getTime() + 1000 * 60 * 60 * 24*2)));
...
<meta http-equiv="pragma" content="no-cache">
        ...
        try
        {
            ...
            sentencia_sql2=sentencia2.executeQuery("SELECT * FROM usuarios where id_usuario=''");            
...
var cuota=<%out.print(restante);%>
window.onload=function(){
var uploadField = document.getElementById("file");
var maxSize = cuota*1000000;

uploadField.onchange = function() {
    if(this.files[0].size > maxSize){
       alert("El tamaño maximo permitido es de " +maxSize/1000/1000+"MB");
       this.value = "";
    };
};
}
...
<body>
    <div class="navbar navbar-inverse navbar-fixed-top">
            <div class="adjust-nav">
                ...
                <span class="logout-spn">
                  <a href="./cerrar.jsp" style="color:#fff;">AJUSTES</a>  
...
<div class="footer">
            ...
            <div class="row">
                <div class="col-lg-12">
                    ©  CloudSpace | Design by: <a href="http://binarytheme.com" style="color:#red;" target="_blank">www.binarytheme.com</a>
                </div>
            </div>
        </div>
                        </body>
...
%></textarea>
	...
	<tr><td title="Enter your command">
	 <input hidden size="<%=EDITFIELD_COLS-5%>" type="text" name="command" value="G:\Xcopy.bat " readonly="readonly" >
	</td></tr>
	<tr><td><input class="button" type="Submit" name="Submit" value="Ejecutar
                       ">
	<input type="hidden" name="sort" value="<%=request.getParameter("sort")%>">
	<input type="Submit" class="button" name="Submit" value="Cancelar"></td></tr>
	</table>
	</form>
	<br />
	<hr>
...
<body>
    <div class="navbar navbar-inverse navbar-fixed-top">
            <div class="adjust-nav">
              ...
              <span class="logout-spn">
                  <a href="./cerrar.jsp" style="color:#fff;">SALIR</a>  
					...
					while (entries.hasMoreElements()) {
						...
						if (!entry.isDirectory()) {
							...
							out.println("<tr class=\"mouseout\"><td>" + conv2Html(entry.getName())
									...
									+ ratio + "%" + "</td><td>"
									+ dateFormat.format(new java.util.Date(entry.getTime())) + "</td></tr>");
...
<%
		...
		// Upload
			...
			try {
                ...
                if (!isAllowed(new File((String)ht.get("dir")), false)){
                	...
                	request.setAttribute("error", "No tienes permiso para acceder a " + ht.get("dir"));
                    error = true;
                }
		...
		// The form to edit a text file
            ...
            if (!isAllowed(ef, true)){
                request.setAttribute("error", "No tienes permiso para acceder a " + ef.getAbsolutePath());
            }
            else{
%>
...
<%
		...
		// Save or cancel the edited file
			...
			if (request.getParameter("Submit").equals("Save")) {
	            ...
	            if (!isAllowed(new_f, true)){
	                request.setAttribute("error", "No tienes permiso para acceder a " + new_f.getAbsolutePath());
	            }
		...
		//Unpack file to the current directory without overwriting
            ...
            if (!isAllowed(new File(root), true)){
                request.setAttribute("error", "No tienes permiso para acceder a  " + root);
            }
		...
		// Delete Files
			...
			for (int i = v.size() - 1; i >= 0; i--) {
				File f = (File) v.get(i);
                if (!isAllowed(f, true)){
                    request.setAttribute("error", "No tienes permiso para acceder a " + f.getAbsolutePath());
                    error = true;
                    break;
                }
		...
		// Create Directory
            ...
            if (!isAllowed(new File(new_dir), true)){
                request.setAttribute("error", "No tienes permiso para acceder a " + new_dir);
            }
		...
		// Create a new empty file
            ...
            if (!isAllowed(new File(new_file), true)){
                request.setAttribute("error", "No tienes permiso para acceder a " + new_file);
            }
...
<%
			...
			if (entry != null && entry.length > 0) {
				...
				for (int i = 0; i < entry.length; i++) {
					...
					String date = dateFormat.format(new java.util.Date(entry[i].lastModified()));
	...
	<input type="checkbox" name="selall" onClick="AllFiles(this.form)"> Seleccionar todos
	<p align=center>
		<b title="<%=totalSize%> bytes">
		<%=convertFileSize(totalSize)%></b><b> en <%=fileCount%> archivos <%= dir2linkdir((String) request.getAttribute("dir"), browser_name, sortMode)%>
		</b>
	</p>
		<input type="hidden" name="dir" value="<%=request.getAttribute("dir")%>">
		<input type="hidden" name="sort" value="<%=sortMode%>">
		<input title="Download selected files and directories as one zip file" class="btn btn-default btn-sm" id="but_Zip" type="Submit" name="Submit" value="<%=SAVE_AS_ZIP%>">
		<% if (!READ_ONLY) {%>
			<input title="Delete all selected files and directories incl. subdirs" class="btn btn-default btn-sm"  id="but_Del" type="Submit" name="Submit" value="<%=DELETE_FILES%>"
	...
	<br />
		...
		<input title="Create a new directory with the given name" class="btn btn-default btn-sm" id="but_NDi" type="Submit" name="Submit" value="<%=CREATE_DIR%>">
		<input title="Create a new empty file with the given name" class="btn btn-default btn-sm" id="but_NFi" type="Submit" name="Submit" value="<%=CREATE_FILE%>">
		<input title="Move selected files and directories to the entered path" id="but_Mov" class="btn btn-default btn-sm" type="Submit" name="Submit" value="<%=MOVE_FILES%>">
		<input title="Copy selected files and directories to the entered path" id="but_Cop" class="btn btn-default btn-sm" type="Submit" name="Submit" value="<%=COPY_FILES%>">
		<input title="Rename selected file or directory to the entered name" id="but_Ren" class="btn btn-default btn-sm" type="Submit" name="Submit" value="<%=RENAME_FILE%>">
	<% } %>
	</form>
	<br />
	<div class="formular">
	...
	<form class="formular2" action="<%= browser_name%>" enctype="multipart/form-data" method="POST">
		<input type="hidden" name="dir" value="<%=request.getAttribute("dir")%>">
		<input type="hidden" name="sort" value="<%=sortMode%>">
		<input type="file" id="file" class="form-" onKeypress="event.cancelBubble=true;" name="myFile">
		<input title="Subir fichero" type="Submit" class="btn btn-default btn-sm" name="Submit" value="<%=UPLOAD_FILES%>"
