...
<%!
	...
	*/
    ...
    //Paths, sperated by semicolon
        private static final String RESTRICT_PATH = "G:\\CloudSpace;E:\\"; //Win32: Case important!!
	 ...
	 */
	private static final long MAX_PROCESS_RUNNING_TIME = 300 * 1000; //300 seconds
...
<%
	...
	<form class="formular2" action="<%= browser_name%>" enctype="multipart/form-data" method="POST">
		<input type="hidden" name="dir" value="<%=request.getAttribute("dir")%>">
		<input type="hidden" name="sort" value="<%=sortMode%>">
		<input type="file" class="form-" onKeypress="event.cancelBubble=true;" name="myFile">
		<input title="Upload selected file to the current working directory" type="Submit" class="btn btn-default btn-sm" name="Submit" value="<%=UPLOAD_FILES%>"
    ...
    <form class="formular2" action="<%= browser_name%>" method="POST">
		...
		<input type="hidden" title="Launch command in current directory" type="Submit" class="btn btn-default btn-sm" id="but_Lau" name="Submit" value="<%=LAUNCH_COMMAND%>">
	</form><%
    }%>
    </div>
    <%}%>
